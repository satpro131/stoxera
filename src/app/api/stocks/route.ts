...
import { getUpstoxQuotes, UPSTOX_RESPONSE_MAP } from '@/utils/upstox';

export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const category = searchParams.get('category'); // 'F&O' or 'Cash'
    ...
    if (token) {
      try {
        ...
        // Find the reverse mapping key from UPSTOX_RESPONSE_MAP
        const upstoxResponseKeys = Object.keys(UPSTOX_RESPONSE_MAP);

        stocks = stocks.map(stock => {
          // Find matching key in Upstox response
          // e.g. for RELIANCE, we look for key "NSE_EQ:RELIANCE"
          const matchingKey = upstoxResponseKeys.find(
            key => UPSTOX_RESPONSE_MAP[key] === stock.symbol
          );
          ...
          if (quote) {
            ...
            // Re-scale support and resistance levels slightly based on spot ratio
        ...
        console.warn('Failed to fetch Upstox quotes, falling back to mock data:', err);
      }
    }

    if (category) {
      stocks = stocks.filter(s => s.category.toUpperCase() === category.toUpperCase());
    }
